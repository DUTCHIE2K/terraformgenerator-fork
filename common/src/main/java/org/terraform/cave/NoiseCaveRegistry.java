...
public class NoiseCaveRegistry {
    ...
    public boolean canGenerateCarve(int x, int y, int z, double height, ChunkCache cache) {
        ...
        float filterSea = getGenerateCarveSeaFilter(x, z, height, cache);
        return canGenerateCarve(x, y, z, height, filterSea);
    }

    public boolean canGenerateCarve(int x, int y, int z, double height, float filterSea) {
        if (!TConfig.areCavesEnabled()) {
            return false;
        }
    ...
    public float getGenerateCarveSeaFilter(int x, int z, double height, ChunkCache cache) {
        return yBarrier(tw, x, (int) height, z, TerraformGenerator.seaLevel, 5, 1, cache);
    }
