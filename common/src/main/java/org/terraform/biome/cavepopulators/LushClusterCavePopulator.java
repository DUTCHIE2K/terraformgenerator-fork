...
public class LushClusterCavePopulator extends AbstractCaveClusterPopulator {
    private static final int SMALL_POOL_RX = 3;
    private static final int SMALL_POOL_RY = 2;
    private static final int SMALL_POOL_RZ = 3;
    private static final int SMALL_POOL_CLEAR_HEIGHT = 4;
    private static final int SMALL_POOL_WATER_SUPPORT_DEPTH = 6;
                        ...
                        boolean boundary)
    {
        ...
        if (!isForLargeCave) {
            ...
            List<SimpleBlock> smallPoolTargets = new ArrayList<>();
            while (target.getY() != ceil.getY()) {
                if (target.getY() - floor.getY() < 3 && GenUtils.chance(1, 700) && canSpawnSmallPool(target, boundary)) {
                    new SphereBuilder(
                            random,
                            target,
                            Material.WATER
            ...
            for (SimpleBlock smallPoolTarget : smallPoolTargets) {
                clearSmallPoolArtifacts(smallPoolTarget);
            }
        }
    ...
    private boolean canSpawnSmallPool(@NotNull SimpleBlock target, boolean boundary) {
        if (boundary) {
            return false;
        }
        for (int nx = -SMALL_POOL_RX; nx <= SMALL_POOL_RX; nx++) {
            for (int ny = -SMALL_POOL_RY; ny <= 0; ny++) {
                for (int nz = -SMALL_POOL_RZ; nz <= SMALL_POOL_RZ; nz++) {
                    double equationResult = Math.pow(nx, 2) / Math.pow(SMALL_POOL_RX, 2)
                                            + Math.pow(ny, 2) / Math.pow(SMALL_POOL_RY, 2)
                                            + Math.pow(nz, 2) / Math.pow(SMALL_POOL_RZ, 2);
                    if (equationResult > 1) {
                        continue;
                    }
                    SimpleBlock rel = target.getRelative(nx, ny, nz);
                    if (!clusterColumns.contains(new CoordPair(rel.getX(), rel.getZ())) && !rel.isSolid()) {
                        return false;
                    }
                }
            }
    ...
    private void clearSmallPoolArtifacts(@NotNull SimpleBlock target) {
        for (int nx = -SMALL_POOL_RX; nx <= SMALL_POOL_RX; nx++) {
            for (int ny = -SMALL_POOL_RY; ny <= SMALL_POOL_CLEAR_HEIGHT; ny++) {
                for (int nz = -SMALL_POOL_RZ; nz <= SMALL_POOL_RZ; nz++) {
                    double horizontalEquationResult = Math.pow(nx, 2) / Math.pow(SMALL_POOL_RX, 2)
                                                     + Math.pow(nz, 2) / Math.pow(SMALL_POOL_RZ, 2);
                    if (horizontalEquationResult > 1) {
                        continue;
                    }
                    boolean belowWater = ny <= 0
                                         && horizontalEquationResult + Math.pow(ny, 2) / Math.pow(SMALL_POOL_RY, 2) <= 1;
                    boolean aboveWater = ny > 0;
                    if (!belowWater && !aboveWater) {
                        continue;
                    }

                    SimpleBlock rel = target.getRelative(nx, ny, nz);
                    if (!hasPoolWaterSupport(rel)) {
                        continue;
                    }

                    if (isSmallPoolVegetation(rel.getType()) || isSmallPoolDecoration(rel.getType())) {
    ...
    private boolean hasPoolWaterSupport(@NotNull SimpleBlock block) {
        SimpleBlock cursor = block;
        for (int i = 0; i < SMALL_POOL_WATER_SUPPORT_DEPTH; i++) {
            if (BlockUtils.isWet(cursor)) {
                return true;
            }
    ...
    private boolean isSmallPoolVegetation(@NotNull Material type) {
        return type == Material.GRASS
               ...
               || type == Material.BIG_DRIPLEAF_STEM;
    }

    private boolean isSmallPoolDecoration(@NotNull Material type) {
        return type == Material.COBBLESTONE_WALL
               || type == Material.MOSSY_COBBLESTONE_WALL
               || type == Material.COBBLED_DEEPSLATE_WALL
               || type == Material.SANDSTONE_WALL
               || type == Material.STONE_SLAB
               || type == Material.COBBLESTONE_SLAB
               || type == Material.COBBLED_DEEPSLATE_SLAB
               || type == Material.MOSSY_COBBLESTONE_SLAB
