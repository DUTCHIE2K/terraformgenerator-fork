...
public abstract class AbstractCaveClusterPopulator extends AbstractCavePopulator {
    ...
    protected final Set<CoordPair> clusterColumns = new HashSet<>();
    protected final Set<String> decoratedPairs = new HashSet<>();
    ...
    public void populate(TerraformWorld tw, Random random, @NotNull SimpleBlock ceil, @NotNull SimpleBlock floor) {
        ...
        while (!queue.isEmpty()) {
            ...
            CoordPair currentColumn = new CoordPair(v.getX(), v.getZ());
            clusterColumns.add(currentColumn);
        ...
        for (int i = 0; i < ceilFloorPairs.size(); i++) {
            ...
            decoratedPairs.add(getDecoratedPairKey(candidates[0], candidates[1]));
            oneUnit(tw, random, candidates[0], candidates[1], boundaries.get(i));
        }
    }

    public @NotNull Set<String> getDecoratedPairs() {
        return Collections.unmodifiableSet(decoratedPairs);
    }

    static @NotNull String getDecoratedPairKey(int x, int z, int ceilY, int floorY) {
        return x + "," + z + "," + ceilY + "," + floorY;
    }

    private static @NotNull String getDecoratedPairKey(@NotNull SimpleBlock ceil, @NotNull SimpleBlock floor) {
        return getDecoratedPairKey(ceil.getX(), ceil.getZ(), ceil.getY(), floor.getY());
    }
}
