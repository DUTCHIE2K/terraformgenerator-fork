        ...
        for (int y = minRadiusY; y <= radiusY; y++) {
            for (int x = -Math.round(radiusX); x <= radiusX; x++) {
                ...
                for (int z = -Math.round(radiusZ); z <= radiusZ; z++) {
                    ...
                    double effectiveYSquared = effectiveY * effectiveY;
                    double equationResult = xSquared / radiusXSquared
                                            + effectiveYSquared / radiusYSquared
                                            + zSquared / radiusZSquared;
                    ...
                    ))
                    {
                        ...
                        if (mangrovePropagules
                            && material == V_1_19.MANGROVE_LEAVES
                            && Version.VERSION.isAtLeast(Version.v1_19_4)
                            && !BlockUtils.isWet(relativeBlock.getDown()))
                        {
