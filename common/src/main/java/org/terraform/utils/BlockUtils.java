...
public class BlockUtils {
    ...
    private static final EnumSet<Material> waterloggableMaterials = EnumSet.noneOf(Material.class);
    ...
    public static void initBlockUtils() {
        ...
        for (Material mat : Material.values()) {
            ...
            if (mat.isBlock()) {
                try {
                    if (Bukkit.createBlockData(mat) instanceof Waterlogged) {
                        waterloggableMaterials.add(mat);
                    }
                }
                catch (IllegalArgumentException ignored) {
                    // Some version-dependent materials can still fail here.
                }
            }
        }
    }
    ...
    public static boolean isWet(@NotNull Material type, @Nullable BlockData data) {
        return BlockUtils.wetMaterials.contains(type) || (data instanceof Waterlogged waterlogged
                                                          && waterlogged.isWaterlogged());
    }

    public static boolean canWaterlog(@NotNull Material type) {
        return waterloggableMaterials.contains(type);
    }

    /**
     * Preserve submerged waterloggable blocks when generation replaces a wet block.
     * Above sea level, this intentionally does nothing.
     */
    public static @NotNull BlockData correctWaterloggedData(@NotNull BlockData data,
                                                            @NotNull Material replacedType,
                                                            @Nullable BlockData replacedData,
                                                            int y)
    {
        if (y > TerraformGenerator.seaLevel
            || !isWet(replacedType, replacedData)
        ...
        BlockData corrected = data.clone();
        ((Waterlogged) corrected).setWaterlogged(true);
        return corrected;
    }
