...
public class GenUtils {
    ...
    public static ConcurrentLRUCache<TWCoordPair, EnumSet<BiomeBank>> biomeQueryCache;
    ...
    public static @NotNull EnumSet<BiomeBank> getBiomesInChunk(TerraformWorld tw, int chunkX, int chunkZ) {
        return biomeQueryCache.get(new TWCoordPair(tw, chunkX, chunkZ));
    }
    ...
    public static int getTransformedHeight(@NotNull TerraformWorld tw, int rawX, int rawZ)
    {
        ...
        if (cachedY == TerraformGeneratorPlugin.injector.getMinY() - 1 && !cache.areTransformedHeightsFilled()) {
            synchronized (cache) {
